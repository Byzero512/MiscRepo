...
# from .exchangeGot_Exception import * 
from mypwn.mypwnlib.Exception.myExcept import *
from .exchangeGot_Var import *
...
def init_gloVar(fpath,arg_offset=1):
    ...
    elf_fp=ELF(fpath)
    section_got_plt_addr=elf_fp.get_section_by_name('.got.plt').header.sh_addr
    try:
        if elf_fp.arch=='i386':
...
def ret_section_plt_got_plt_addr_dict():
    ...
    for i in range(symbol_num):
        ...
        symbol_got_plt_addr_list.append(section_got_plt_addr+(i+3)*symbol_got_plt_sz)
...
def set_symbol_dynsym_index_list():
    ...
    symbol_rel_plt_list=ret_list_base_len(section_rel_plt_data,symbol_rel_plt_sz)
...
def ret_symbol_dynsym_dict():
    ...
    section_dynsym_data_list=ret_list_base_len(section_dynsym_data,symbol_dynsym_sz)
...
def ret_symbol_gnu_version_dict():
    ...
    section_gnu_version_data_list=ret_list_base_len(section_gnu_version_data,2)
...
def ret_jmp_machine_code(src_addr,target_addr):
    if arch=='amd64':
        ...
        if elf_fp.pie:
            jmp_machine_code='\xff\xa3{}'.format(p32(target_addr-section_got_plt_addr))
            return jmp_machine_code
        else:
...
def exchange_plt():
    ...
    symbol_got_plt_addr_list=symbol_got_plt_addr_list[offset:]+symbol_got_plt_addr_list[:offset]
    for i in range(symbol_num):
...
def exchange_gnu_version():
    ...
    symbol_gnu_version_data_list=list_move_right(symbol_gnu_version_data_list)

    for i in range(symbol_num):
        elf_fp.write(symbol_gnu_version_addr_list[i],symbol_gnu_version_data_list[i])
